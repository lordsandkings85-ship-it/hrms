...
export class RecruitmentService {
  ...
  listJobs(companyId: string) {
    return this.prisma.job.findMany({ where: { companyId }, include: { candidates: { include: { interview: true, offer: true } } } });
  }
  ...
  async removeCandidate(candidateId: string) {
    ...
    await this.prisma.offer.deleteMany({ where: { candidateId } });
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.salaryStructure.create({
        data: {
          ...
          basic: Math.round((offer.ctc / 12) * 0.5),
          hra: Math.round((offer.ctc / 12) * 0.25),
          specialAllowance: Math.max(0, Math.round((offer.ctc / 12) - ((offer.ctc / 12) * 0.5) - ((offer.ctc / 12) * 0.25))),
          pfDeduction: Math.round((offer.ctc / 12) * 0.5) * 0.12,
          ptDeduction: 0,
