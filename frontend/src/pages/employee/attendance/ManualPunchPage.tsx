...
import { Fingerprint, CheckSquare, Loader2, Save } from 'lucide-react';
...
export default function ManualPunchPage() {
  ...
  const onSubmit = async (data: PunchData) => {
    ...
    try {
      await attendanceApi.manualPunch({ employeeId: myEmpId, date: data.date, time: data.time, type: data.type, reason: data.reason });
           ...
           <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <label className="flex-1">
                  ...
                  <div className="w-full p-4 border border-[var(--border)] rounded-xl text-center font-bold text-[var(--text-muted)] peer-checked:bg-fuchsia-500/10 peer-checked:text-fuchsia-500 peer-checked:border-fuchsia-500 cursor-pointer transition-colors">
                    Punch IN
                  </div>
                </label>
                <label className="flex-1">
