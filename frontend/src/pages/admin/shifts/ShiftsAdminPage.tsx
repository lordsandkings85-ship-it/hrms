...
import { Clock, Plus, Loader2, Check, Users, X, Trash2 } from 'lucide-react';
...
export default function ShiftsAdminPage() {
  ...
  const create = useMutation({
    mutationFn: () => shiftsApi.create({ name, startTime: start, endTime: end, type: 'regular' }),
    onSuccess: () => { success('Shift created'); setName(''); invalidate(); },
  ...
  const shiftColumns: Column<any>[] = [
    ...
    { key: 'start', header: 'Start', render: (r: any) => <span className="font-mono">{formatTime12(r.startTime)}</span> },
    { key: 'end', header: 'End', render: (r: any) => <span className="font-mono">{formatTime12(r.endTime)}</span> },
    { key: 'type', header: 'Type', render: (r: any) => <span className="text-[var(--text-muted)] text-xs capitalize">{r.type}</span> },
    {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">Define shifts and assign employees.</p>
      </div>
      <AdminSection
        title="Define Shift"
        icon={Clock}
        ...
        right={
          <div className="flex items-end gap-2 flex-wrap">
            ...
            <input type="time" value={start} onChange={(e) => setStart(e.target.value)} className="px-3 py-2 bg-[var(--surface-alt)] border border-[var(--border)] rounded-xl text-sm focus:outline-none focus:border-indigo-500" />
            <input type="time" value={end} onChange={(e) => setEnd(e.target.value)} className="px-3 py-2 bg-[var(--surface-alt)] border border-[var(--border)] rounded-xl text-sm focus:outline-none focus:border-indigo-500" />
