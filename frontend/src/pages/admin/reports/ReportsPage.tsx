...
export default function ReportsPage() {
  ...
  const { data: payrollMonthly } = useQuery({ queryKey: ['report-payroll-monthly', selectedYear], queryFn: () => reportsApi.payrollCostMonthly(selectedYear) });
  ...
  const monthlyTotal = payrollMonthly?.months?.reduce((s: number, m: any) => s + m.totalNet, 0) ?? 0;
  const hasMonthlyData = monthlyTotal > 0;
  const payrollCostTimeline = (payrollMonthly?.months ?? []).map((m: any) => ({
    month: new Date(2000, m.month - 1, 1).toLocaleString('default', { month: 'short' }),
    cost: m.totalNet ?? 0,
  }));
  ...
  return (
    <div className="page-container max-w-7xl space-y-6">
      <div className="animate-slideUp" style={{ animationDelay: '0.1s' }}>
        <PageHeader
          title="Report Builders & Analytical Insights"
          subtitle="Custom Enterprise Reports · Employee Master · Monthly Attendance · Payroll Summary"
          icon={BarChart3}
          actions={
      ...
      {/* TABS for Report Builders */}
      <div className="tab-container animate-slideUp overflow-x-auto flex gap-2 pb-2" style={{ animationDelay: '0.12s' }}>
        {[
          { key: 'employee-info', label: 'Employee Information', path: '/reports/employee-info', icon: <Users size={15} /> },
          { key: 'attendance', label: 'Monthly Attendance', path: '/reports/attendance', icon: <Clock size={15} /> },
          { key: 'salary', label: 'Monthly Salary', path: '/reports/salary', icon: <DollarSign size={15} /> },
          { key: 'salary-summary', label: 'Monthly Salary Summary', path: '/reports/salary-summary', icon: <Wallet size={15} /> },
          { key: 'salary-structure', label: 'Employee Salary Structure', path: '/reports/salary-structure', icon: <FileText size={15} /> },
        ].map(t => {
          const isActive = (activeSub === 'employee-info' && t.key === 'employee-info') || activeSub === t.key;
          return (
            <button
              key={t.key}
              onClick={() => navigate(t.path)}
              className={`tab-pill flex items-center gap-2 whitespace-nowrap text-xs ${
